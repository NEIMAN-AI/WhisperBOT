...
fuckufffXD = Client("WHISPER-BOT", api_id=API_ID, api_hash=API_HASH, bot_token=BOT_TOKEN)

ALPHA = {}

STXT = "Bot started successfully ✨🥀\n"
STXT += "For queries and questions : @fuckufffXD\n"
STXT += "Join @Neiman_x_support for community ✨☁️"
...
@fuckufffXD.on_message(filters.command("start") & filters.private)
async def start(_, m):
...
@fuckufffXD.on_message(filters.command("help") & filters.private)
async def help(_, m):
    global un
    if not un:
